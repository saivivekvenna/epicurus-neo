...
| M7 | **Dynamic upstream gate** (layered safe-rejection) | Built a label-blind selective-prediction gate (`configs/frozen/dynamic_gate_v1.json`, `dynamic-gate` CLI, 16 safety tests): Layer0 deterministic impossibility + Layer1 AND-of-core-vetoes `{el,prime}` (missing→KEEP) + Layer2 expression rescue-only + Layer3 patient-adaptive rails; CP-lower-bound (Neyman–Pearson) LOCO calibration; frozen before LOCKED CheckMate 153. **Feasibility:** the AND-gate **Pareto-dominates** the incumbent pure-EL gate on positive retention at matched removal. **But the aggressive premise is FALSIFIED:** (a) 50–75% removal at ≥95% retention is unreachable on peptide-only features (IMPROVE positives spread → retention collapses); (b) at any SAFE retention (≈1.0) downstream Δhits@20 = **0** — the gate removes only sub-top-20 negatives and **0%** of the high-presentation decoys that outrank positives (the ones the oracle's random pruning deletes for its 0.808→1.652 lift). The oracle gap is **unclosable by any label-blind presentation gate** = the recognition wall re-derived via the gate/oracle decomposition. CP-LB is sample-size-capped (46 pos ⇒ max 0.937). Honest value = **recall-preserving candidate-universe pruning** (CheckMate: 10.4% neg removed @ 95.7% retention, no regression), NOT top-20 lift. Unlock = mutant-RNA-VAF / DNA-VAF+depth+purity/CCF / processing / agretopicity as KEEP-only rescue axes (Miller IPV `PRJNA980652`, Gartner reconstruction). | B (safe pruner) / C (top-20 premise) | `artifacts/milestone_7_decision/dynamic_gate/{SPEC,FEASIBILITY,REPORT,VERDICT,FALSIFICATION_LEDGER}.md`, `src/event_b/dynamic_gate.py`, `scripts/dynamic_gate.py`, `configs/frozen/dynamic_gate_v1.json` |
| M7 | **Lossless input-only peptide recovery** (Phase 2/3 impl + online/offline run) | Frozen input-only generator (`lossless-peptide-generation-1.0.0`): raw GRCh38 allele → Ensembl VEP MANE/canonical (URL+SHA offline cache, fail-closed) → CDS/protein → all standard-AA 8–14 windows spanning the mutant/novel-frame residue → HLA panel from pVAC → **genuine PRIME** → union with pVAC (+`_cache_prime`) on stable genomic identity → frozen router route-aware top-20 (`genuine_prime = −PRIME %rank`). Reads NO assay/vaccine/label input (test-enforced); Hudson labels joined only AFTER ranking (eval-only). **Reproduced the disclosed post-hoc feasibility exactly:** ASPM **77** windows (best %rank **0.088**), MAP2 Gly868fs **259** windows (**0.010**), DYNC1H1 control **77** windows reproducing pVAC `KRFHATISF` (0.002). Adding the recovered candidates moves the 3-target coverage from **1/3 → 3/3** at every stage (candidate-gen recall, rankable recall, pure-PRIME top-20, route-aware top-20). **Online == offline** identical (`content_hash 74bbd8fd…`; offline serves the gitignored Ensembl cache and fails closed on a miss). | reachability fix; **NO superiority** (better generation feeds genuine PRIME targets it never received) | `artifacts/milestone_7_decision/peptide_recovery/{RECOVERED_CANDIDATES.csv,TOP20.csv,RESULT.json,REPORT.md,PROVENANCE.json,NORTH_STAR_FINAL.md}`, `src/event_b/lossless_peptide_generation.py`, `scripts/osteosarc_peptide_recovery.py` |
