...
- **MAP2 G868FS** — MAP2 frameshift (Hudson label p.GYCVFNKYTV868fs, recognized May): the site carries two overlapping frameshift annotations ['MAP2-chr2-209694768(p.Leu867fs)', 'MAP2-chr2-209694772(p.Gly868fs)'] 4bp apart. Whether these are alternate representations of one event or distinct overlapping deletions cannot be resolved from the public tables. The vaccine (JLF V1/V2/V3) + ELISPOT-strong record is on MAP2-chr2-209694768 (p.Leu867fs); the Hudson '868fs' label position-matches MAP2-chr2-209694772 (p.Gly868fs, no vaccine/experiments). The frameshift neo-peptide GYCVFNKYTV is not present in either page's blocks, so sequence cannot disambiguate. BOTH are called by DRAGEN/Sarek/oncoanalyser and BOTH are absent from the pVACtools 2025 candidate universe -> lost at candidate generation. The prior 'low-TPM expression-filter drop' attribution is not confirmable from these files (MAP2 is simply absent from the pVACtools output).

## 5. Changes justified for Epicurus (proposed, NOT fit to this patient)

1. **Multi-caller / longitudinal union at candidate generation.** 2 of 3 recognized, vaccine-included, ELISPOT-positive neoantigens (ASPM, MAP2) were *called by DRAGEN/Sarek/oncoanalyser* yet dropped by the single pVACtools 2025.01 candidate step. The recoverable loss is candidate **recall**, upstream of any ranker.
