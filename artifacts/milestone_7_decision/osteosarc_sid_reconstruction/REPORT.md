...
- **ASPM G2179R** — ASPM p.G2179R (Hudson-recognized May+Aug) = site variant ASPM-chr1-197102716 (p.Gly2179Arg), called by DRAGEN/Sarek/oncoanalyser. The page lists an mRNA peptide construct but zero vaccines targeting it and zero site-ELISPOT experiments; the Hudson recognition stream is the positive evidence. It is ABSENT from the pVACtools 2025.01 candidate universe (no ASPM). Lost at candidate generation, NOT variant calling — corrects dd3efd1's 'off-callset' claim.
...
- **MAP2 GYCVFNKYTV868FS** — MAP2 frameshift (Hudson label p.GYCVFNKYTV868fs, recognized May): the site carries two overlapping frameshift annotations ['MAP2-chr2-209694768(p.Leu867fs)', 'MAP2-chr2-209694772(p.Gly868fs)'] 4bp apart. They have different genomic alleles and remain distinct variant records. Ensembl VEP for the Gly868 allele (MANE ENST00000682079, c.2603_2630del, p.Gly868AlafsTer38) predicts a mutant ORF sharing HCHHLFKTVRIYQGRVVPFTKAL with the Leu867-associated construct. The vaccine (JLF V1/V2/V3) + ELISPOT-strong record is on MAP2-chr2-209694768 (p.Leu867fs); the Hudson '868fs' label position-matches MAP2-chr2-209694772 (p.Gly868fs, no vaccine/experiments). VEP identifies GYCVFNKYTV as the reference amino-acid context, so interpreting the Hudson label as reference-context naming rather than a mutant epitope is an inference, not an assay fact. BOTH variants are called by DRAGEN/Sarek/oncoanalyser and absent from pVACtools 2025 candidate universe -> lost at candidate generation. The prior 'low-TPM expression-filter drop' attribution is not confirmable from these files (MAP2 is simply absent from the pVACtools output).
