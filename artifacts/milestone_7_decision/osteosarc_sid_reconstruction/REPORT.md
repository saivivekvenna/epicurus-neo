...
- **MAP2 G868FS** — MAP2 frameshift (Hudson label p.GYCVFNKYTV868fs, recognized May) maps by residue-868 to a site MAP2 frameshift; the site carries two frameshift annotations ['MAP2-chr2-209694768(p.Leu867fs)', 'MAP2-chr2-209694772(p.Gly868fs)'] 4bp apart (caller-dependent 867fs vs 868fs of one event), both called by DRAGEN/Sarek/oncoanalyser and BOTH absent from the pVACtools 2025 candidate universe. Lost at candidate generation. The prior 'low-TPM expression-filter drop' attribution is not confirmable from these files (it is simply absent from the pVACtools output).
