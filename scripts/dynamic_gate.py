...
_VERDICT = (
    "**B for the gate as a safe recall-preserving pruner. C is SCOPED: falsified only for "
    "same-feature presentation/PRIME-derived monotone gating with current peptide features. The general "
    "orthogonal-feature dynamic-gate hypothesis is UNTESTED / data-blocked — NOT falsified.**\n\n"
    "The layered gate is real, safe, and Pareto-dominates the incumbent EL-percentile gate on positive "
    "retention at matched removal (FEASIBILITY.md). Its honest value is **shrinking the candidate universe "
    "handed to expensive downstream steps (genuine-PRIME scoring, wet-lab) without losing recognized "
    "positives** — NOT lifting top-20.\n\n"
    "**Circularity caveat (CIRCULARITY_AUDIT.md).** The v1 gate's veto axes {EL, PRIME} ARE the dominant "
    "inputs to the downstream rankers (genuine PRIME uses PRIME; frozen Epicurus is PRIME-dominated over "
    "prime/el/expr). So the gate removes exactly the candidates those same rankers already bury: 0 "
    "gate-removed candidates ever sat in a ranker top-20, and it removes **0%** of the high-EL/high-PRIME "
    "decoy stratum by construction. The zero downstream Δhits@20 at safe retention is therefore a "
    "**structural tautology of feature overlap**, and proves only that a same-feature monotone "
    "presentation gate cannot move a same-feature top-20 — it does NOT prove a label-blind gate in general "
    "cannot help. The aggressive-gating premise is falsified for this feature class only.\n\n"
    "CP retention lower bounds are sample-size-capped (Gartner 46 positives ⇒ max CP-LB 0.937), so small "
    "cohorts cannot certify ≥0.95 regardless of the gate — an underpowering limit, not an unsafety.\n\n"
    "**What could close the gap (v2, data-blocked; V2_CONTRACT.md / V2_PREREGISTRATION.md):** a gate whose "
    "veto uses features ORTHOGONAL to the downstream rank — mutant-allele RNA VAF + read support, tumor "
    "DNA VAF/depth/CCF/clonality, proteasomal processing/stability, agretopicity/WT-differential, "
    "transcript confidence — as cross-fitted residual signals after conditioning on PRIME/EL (which define "
    "only the hard-decoy stratum + rescue/floor, never veto axes). Missing⇒KEEP. **Constraints learned:** "
    "(i) a SEQUENCE-ONLY residual gate is FALSIFIED — an independent train-Gartner→test-IMPROVE run retained "
    "1.5% of positives (peptide/HLA motifs encode severe study/assay shift); (ii) leave-one-STUDY-out "
    "calibration + explicit OOD abstention are mandatory. An in-sample multimer probe shows only a faint "
    "orthogonal signal (AUROC ~0.59), unvalidatable cross-study here. Concrete path = open WES+RNA of "
    "Miller IPV (PRJNA980652) + Gartner reconstruction."
)
